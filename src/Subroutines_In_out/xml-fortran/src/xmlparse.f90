...
subroutine xml_open( info, fname, mustread )
   ...
   write (6, *) ' In xml_open'
   do i = 10,99
       write (6, *) ' Checking unit: ', i
      inquire( unit = i, opened = opend )
      if ( .not. opend ) then
         info%lun = i
         write (6, *) ' info%lun = ', info%lun
         ...
         if ( .not. exists .and. mustread ) then
            ...
            info%error = .true.
             ...
             write (6, *) ' About to open file: ', fname
            open( unit = info%lun, file = fname, iostat = ierr )
            if (ierr.ne.0) then
                write (6, *) 'File not found. Ierr=', ierr
                stop
            endif
             write (6, *) ' File opened '
            call xml_report_details( 'XML_OPEN: opened file ', trim(fname) )
            call xml_report_details( 'at LU-number: ', info%lun )
             write (6, *) ' called xml_report_details twice '
         endif
         exit
      endif
   enddo
   write (6, *) ' Opened xml file'
